...
var transURL = arkPath + "/autocomplete";		// trans 페이지의 URL을 설정한다.
var tempQuery = "";
...
function drawArk() {
	...
	str += "		<div class=\"bottom\" id=\"" + footerId +"\"></div>";
	str += "</div>";
	...
	$.ajax({
		...
		success: function(data) {
			...
			$.each(data.result, function(i, result) {
				...
				if (totalCount > 0) {
					...
					$.each(result.items, function(num,item){
						...
						str += " onmouseout=\"onMouseOutKeyword(" + num + ");\"><a href=\"#none#\" onclick=\"onClickKeyword(" + num + ");\" >" + item.hkeyword;
						str += " </a><span id=\"f" + num + "\" style=display:none;>" + item.keyword + "</span>";
						str += " </li>";
					});
				}
			});
			...
			if ((totalFwCount + totalRwCount) == 0) {
				if($("#searchLang").val()== "ko"){
					$("#" + contentListId).html("<li><a href=\"#none#\">해당 단어로 시작하는 검색어가 없습니다.</a></li>");
				}else{
					$("#" + contentListId).html("<li><a href=\"#none#\">No terms that begin with those words.</a></li>");
				}
			} else {
...
function setArkFooter() {
	...
	if (isArk && $(queryId).val() != "" && (totalFwCount + totalRwCount) > 0 && isListShow) {
		...
		if (g_oConvert == "fw") {
			if($("#searchLang").val() == "ko"){
					str += "<a OnClick=\"onConvert('rw');\" class=\"btn-off\" style=\"cursor:pointer;\">끝단어 더보기</a>";
			}else{
					str += "<a OnClick=\"onConvert('rw');\" class=\"btn-off\" style=\"cursor:pointer;\">Words ending more</a>";
			}
			
		} else if (g_oConvert == "rw") {
			if($("#searchLang").val() == "ko"){
					str += "<a OnClick=\"onConvert('fw');\" class=\"btn-off\" style=\"cursor:pointer;\">첫단어 더보기</a>";
			}else{
					str += "<a OnClick=\"onConvert('rw');\" class=\"btn-off\" style=\"cursor:pointer;\">The first word more</a>";
			}
